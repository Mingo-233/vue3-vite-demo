var opentype = (() => {
  ...
  encode.CHARARRAY = function (v) {
    if (v === null || typeof v === "undefined") {
      v = "";
      console.warn(
        "CHARARRAY with undefined or null value encountered and treated as an empty string. This is probably caused by a missing glyph name."
      );
    }
    const b = [];
  ...
  encode.FLOAT = function (v) {
    if (v > MAX_16_16 || v < MIN_16_16) {
      throw new Error(
        `Value ${v} is outside the range of representable values in 16.16 format`
      );
    }
  ...
  function ClassDef(classDefTable) {
    if (classDefTable.format === 1) {
      ...
      Table.call(
        this,
        "classDefTable",
        [{ name: "classFormat", type: "USHORT", value: 2 }].concat(
          recordList(
            "rangeRecord",
  ...
  Parser.prototype.parseItemVariationStore = function () {
    ...
    const itemVariationDataOffsets = this.parseULongList(
  ...
  Parser.prototype.parseItemVariationSubtable = function () {
    ...
    const subtable = {
      regionIndexes,
      deltaSets:
        itemCount && regionIndexCount
          ? this.parseDeltaSets(itemCount, wordDeltaCount, regionIndexCount)
          : [],
    };
    return subtable;
  };
  ...
  Parser.prototype.parseDeltaSets = function (
    itemCount,
    wordDeltaCount,
    regionIndexCount
  ) {
  ...
  Parser.prototype.parseTupleVariationStoreList = function (
    axisCount,
    flavor,
    glyphs
  ) {
    ...
    const parseOffset = (
      offsetSizeIs32Bit ? this.parseULong : this.parseUShort
    ).bind(this);
  ...
  Parser.prototype.parseTupleVariationStore = function (
    tableOffset,
    axisCount,
    flavor,
    glyphs,
    ...
    const hasSharedPoints = !!(
      tupleVariationCount & masks.SHARED_POINT_NUMBERS
    );
    ...
    if (this.relativeOffset !== tableOffset + dataOffset) {
      console.warn(
        `Unexpected offset after parsing tuple variation headers! Expected ${
          tableOffset + dataOffset
        }, actually ${this.relativeOffset}`
      );
    ...
    for (let h = 0; h < count; h++) {
      ...
      if (flavor === "cvar" && !header.peakTuple) {
        console.warn(
          "An embedded peak tuple is required in TupleVariationHeaders for the cvar table."
        );
      }
      ...
      Object.defineProperty(
        header,
        "deltas",
        defineDeltas.call(this, "deltas")
      );
      if (flavor === "gvar") {
        Object.defineProperty(
          header,
          "deltasY",
          defineDeltas.call(this, "deltasY")
        );
      }
  ...
  Parser.prototype.parseTupleVariationHeader = function (axisCount, flavor) {
    ...
    const sharedTupleRecordsIndex = embeddedPeakTuple
      ? void 0
      : tupleIndex & masks.TUPLE_INDEX_MASK;
    const peakTuple = embeddedPeakTuple
      ? this.parseTupleRecords(1, axisCount)[0]
      : void 0;
    const intermediateStartTuple = intermediateRegion
      ? this.parseTupleRecords(1, axisCount)[0]
      : void 0;
    const intermediateEndTuple = intermediateRegion
      ? this.parseTupleRecords(1, axisCount)[0]
      : void 0;
    const result = {
      variationDataSize,
  ...
  Parser.prototype.parsePackedPointNumbers = function () {
    ...
    if (countByte1 >= 128) {
      ...
      totalPointCount =
        ((countByte1 & masks.POINT_RUN_COUNT_MASK) << 8) | countByte2;
    }
    let lastPoint = 0;
  ...
  function getNameByID(names, nameID, allowedStandardIDs = []) {
    if (nameID < 256 && nameID in nameTableNames) {
      if (
        allowedStandardIDs.length &&
        !allowedStandardIDs.includes(parseInt(nameID))
      ) {
        return void 0;
      }
  ...
  function parseCmapTableFormat4(cmap, p, data, start, offset) {
    ...
    const startCountParser = new parse_default.Parser(
      data,
      start + offset + 16 + segCount * 2
    );
    const idDeltaParser = new parse_default.Parser(
      data,
      start + offset + 16 + segCount * 4
    );
    const idRangeOffsetParser = new parse_default.Parser(
      data,
  ...
  function parseCmapTable(data, start) {
    ...
    for (let i = cmap.numTables - 1; i >= 0; i -= 1) {
      ...
      if (
        (platformId === 3 && platform3Encodings.includes(encodingId)) ||
    ...
    if (cmap.format === 0) {
      ...
      throw new Error(
        "Only format 0 (platformId 1, encodingId 0), 4, 12 and 14 cmap tables are supported (found format " +
          cmap.format +
          ", platformId " +
          platformId +
          ", encodingId " +
  ...
  function makeCpalTable({
    ...
    check_default.argument(
      colorRecordIndices.length,
      "No colorRecordIndices given."
    );
  ...
  function getPaletteColor(font, index, palette = 0, colorFormat = "hexa") {
    ...
    if (palette > cpalTable.colorRecordIndices.length - 1) {
      throw new Error(
        `Palette index out of range (colorRecordIndices.length: ${cpalTable.colorRecordIndices.length}, index: ${index})`
      );
    }
    if (index > cpalTable.numPaletteEntries) {
      throw new Error(
        `Color index out of range (numPaletteEntries: ${cpalTable.numPaletteEntries}, index: ${index})`
      );
    }
    const lookupIndex = cpalTable.colorRecordIndices[palette] + index;
    if (lookupIndex > cpalTable.colorRecords) {
      throw new Error(
        `Color index out of range (colorRecords.length: ${cpalTable.colorRecords.length}, lookupIndex: ${lookupIndex})`
      );
    }
  ...
  function bgraToRaw(color) {
    return parseInt(
      `0x${toHex(color.b)}${toHex(color.g)}${toHex(color.r)}${toHex(
        color.a * 255
      )}`,
      16
    );
  ...
  function parseColor(color, targetFormat = "hexa") {
    ...
    if ((isRaw && returnRaw) || color === "currentColor") {
      ...
      !isRaw &&
      /^#([a-f0-9]{3}|[a-f0-9]{4}|[a-f0-9]{6}|[a-f0-9]{8})$/i.test(color.trim())
    ) {
      ...
      color = color.trim();
      const rgbaRegex =
        /rgba?\(\s*(?:(\d*\.\d+)(%?)|(\d+)(%?))\s*(?:,|\s*)\s*(?:(\d*\.\d+)(%?)|(\d+)(%?))\s*(?:,|\s*)\s*(?:(\d*\.\d+)(%?)|(\d+)(%?))\s*(?:(?:,|\s|\/)\s*(?:(0*(?:\.\d+)?()|0*1(?:\.0+)?())|(?:\.\d+)|(\d+)(%)|(\d*\.\d+)(%)))?\s*\)/;
      if (rgbaRegex.test(color)) {
        ...
        const hslaRegex =
          /hsla?\(\s*(?:(\d*\.\d+|\d+)(deg|turn|))\s*(?:,|\s*)\s*(?:(\d*\.\d+)%?|(\d+)%?)\s*(?:,|\s*)\s*(?:(\d*\.\d+)%?|(\d+)%?)\s*(?:(?:,|\s|\/)\s*(?:(0*(?:\.\d+)?()|0*1(?:\.0+)?())|(?:\.\d+)|(\d+)(%)|(\d*\.\d+)(%)))?\s*\)/;
        if (hslaRegex.test(color)) {
          ...
          color = hslToRGB({
            ...
            a: !matches[5]
              ? 1
              : parseFloat(matches[5]) / (matches[6] ? 100 : 1),
          });
        } else {
          validFormat = false;
        }
  ...
  function formatColor(bgra, format = "hexa") {
    ...
    switch (format) {
      case "rgba":
        return `rgba(${bgra.r}, ${bgra.g}, ${bgra.b}, ${parseFloat(
          bgra.a.toFixed(3)
        )})`;
      case "rgb":
      ...
      case "hex8":
      case "hex-8":
        return `#${toHex(bgra.r)}${toHex(bgra.g)}${toHex(bgra.b)}${toHex(
          bgra.a * 255
        )}`;
      case "hsl":
        return `hsl(${hsl.h.toFixed(2)}, ${hsl.s.toFixed(2)}%, ${hsl.l.toFixed(
          2
        )}%)`;
      case "hsla":
        return `hsla(${hsl.h.toFixed(2)}, ${hsl.s.toFixed(2)}%, ${hsl.l.toFixed(
          2
        )}%, ${parseFloat(bgra.a.toFixed(3))})`;
      case "bgra":
        return bgra;
      case "raw":
      case "cpal":
  ...
  Glyph.prototype.bindConstructorValues = function (options) {
    ...
    this.unicodes =
      options.unicodes || (options.unicode !== void 0 ? [options.unicode] : []);
    if ("xMin" in options) {
    ...
    if (options.drawLayers) {
      ...
      if (layers && layers.length) {
        ...
        for (let i = 0; i < layers.length; i += 1) {
          ...
          p._layers.push(
            this.getPath.call(layer.glyph, x, y, fontSize, options, font)
          );
        }
        return p;
      }
  ...
  Glyph.prototype.getLayers = function (font) {
    if (!font) {
      throw Error(
        "The font object is required to read the colr/cpal tables in order to get the layers."
      );
    }
  ...
  Glyph.prototype.getSvgImage = function (font) {
    if (!font) {
      throw Error(
        "The font object is required to read the svg table in order to get the image."
      );
    }
  ...
  Glyph.prototype.drawPoints = function (ctx, x, y, fontSize, options, font) {
    ...
    if (font && font.variation) {
      commands = font.variation.getTransform(this, options.variation).path
        .commands;
    }
    for (let i = 0; i < commands.length; i += 1) {
      const cmd = commands[i];
  ...
  Glyph.prototype.toPathData = function (options, font) {
    options = Object.assign(
      {},
      { variation: font && font.defaultRenderOptions.variation },
      options
    );
    let useGlyph = this;
    if (font && font.variation) {
    ...
    let usePath =
      useGlyph.points && options.pointsTransform
        ? options.pointsTransform(useGlyph.points)
        : useGlyph.path;
  ...
  function parseCFFIndex(data, start, conversionFn, version) {
    ...
    const count =
      version > 1
        ? parse_default.getULong(data, start)
        : parse_default.getCard16(data, start);
    ...
    for (let i = 0; i < offsets.length - 1; i += 1) {
      let value = parse_default.getBytes(
        data,
    ...
    for (let i = 0; i < meta.length; i += 1) {
      ...
      if (Array.isArray(m.type)) {
        ...
        for (let j = 0; j < m.type.length; j++) {
          ...
          if (value === void 0) {
            value =
              m.value !== void 0 && m.value[j] !== void 0 ? m.value[j] : null;
          }
          if (m.type[j] === "SID") {
  ...
  function parseCFFHeader(data, start) {
    ...
    if (header.formatMajor > 2) {
      throw new Error(
        `Unsupported CFF table version ${header.formatMajor}.${header.formatMinor}`
      );
    }
  ...
  function parseCFFTopDict(data, start, strings, version) {
    ...
    return interpretDict(
      dict,
      version > 1 ? TOP_DICT_META_CFF2 : TOP_DICT_META,
      strings
    );
  }
  function parseCFFPrivateDict(data, start, size, strings, version) {
    ...
    return interpretDict(
      dict,
      version > 1 ? PRIVATE_DICT_META_CFF2 : PRIVATE_DICT_META,
      strings
    );
  }
  ...
  function applyPaintType(font, path) {
    const paintType =
      (font.tables.cff &&
        ...
        font.tables.cff.topDict.paintType) ||
      0;
    if (paintType === 2) {
      path.fill = null;
      path.stroke = "black";
  ...
  function parseCFFCharstring(font, glyph, code, version, coords) {
    ...
    if (font.isCIDFont || version > 1) {
      const fdIndex = cffTable.topDict._fdSelect
        ? cffTable.topDict._fdSelect[glyph.index]
        : 0;
      ...
      while (i < code2.length) {
        ...
        switch (v) {
          ...
          case 11:
            if (version > 1) {
              console.error(
                "CFF CharString operator return (11) is not supported in CFF2"
              );
              break;
            }
            return;
          ...
          case 14:
            if (version > 1) {
              console.error(
                "CFF CharString operator endchar (14) is not supported in CFF2"
              );
              break;
            }
            if (stack.length >= 4) {
              ...
              if (acharName && bcharName) {
                ...
                glyph.components = [];
                const acharGlyphIndex =
                  font.cffEncoding.charset.indexOf(acharName);
                const bcharGlyphIndex =
                  font.cffEncoding.charset.indexOf(bcharName);
                ...
                const shiftedCommands = JSON.parse(
                  JSON.stringify(acharGlyph.path.commands)
                );
          ...
          case 15:
            if (version < 2) {
              console.error(
                "CFF2 CharString operator vsindex (15) is not supported in CFF"
              );
              break;
            }
            vsindex = stack.pop();
          ...
          case 16:
            if (version < 2) {
              console.error(
                "CFF2 CharString operator blend (16) is not supported in CFF"
              );
              break;
            }
            if (!blendVector) {
              blendVector =
                font.variation &&
                coords &&
                font.variation.process.getBlendVector(vstore, vsindex, coords);
            }
            var n = stack.pop();
            var axisCount = blendVector
              ? blendVector.length
              : vstore.itemVariationSubtables[vsindex].regionIndexes.length;
  ...
  function parseCFFFDSelect(data, start, nGlyphs, fdArrayCount, version) {
    ...
    if (format === 0) {
      ...
      if (first !== 0) {
        throw new Error(
          `CFF Table CID Font FDSelect format ${format} range has bad initial GID ${first}`
        );
      }
      let next;
      for (let iRange = 0; iRange < nRanges; iRange++) {
        ...
        if (next > nGlyphs) {
          throw new Error(
            `CFF Table CID Font FDSelect format ${version} range has bad GID ${next}`
          );
        }
      ...
      if (next !== nGlyphs) {
        throw new Error(
          "CFF Table CID Font FDSelect format 3 range has bad final (Sentinal) GID " +
            next
        );
      }
    } else {
  ...
  function parseCFFTable(data, start, font, opt) {
    ...
    const nameIndex =
      header.formatMajor > 1
        ? null
        : parseCFFIndex(data, header.endOffset, parse_default.bytesToString);
    const topDictIndex =
      header.formatMajor > 1 ? null : parseCFFIndex(data, nameIndex.endOffset);
    const stringIndex =
      header.formatMajor > 1
        ? null
        : parseCFFIndex(
            ...
            parse_default.bytesToString
          );
    const globalSubrIndex = parseCFFIndex(
      data,
      header.formatMajor > 1
        ? start + header.size + header.topDictLength
    ...
    if (header.formatMajor > 1) {
      ...
      const topDictData = parse_default.getBytes(
        data,
        topDictOffset,
        topDictOffset + header.topDictLength
      );
      topDict = gatherCFFTopDicts(
        data,
        0,
        [topDictData],
        void 0,
    ...
    if (
      header.formatMajor < 2 &&
      topDict.ros[0] !== void 0 &&
      topDict.ros[1] !== void 0
    ) {
      font.isCIDFont = true;
    }
    if (header.formatMajor > 1) {
      ...
      if (!fdArrayIndexOffset) {
        throw new Error(
          "This is a CFF2 font, but FDArray information is missing"
        );
      }
      const fdArrayIndex = parseCFFIndex(
        data,
        start + fdArrayIndexOffset,
        null,
        header.formatMajor
      );
    ...
    if (opt.lowMemory) {
      ...
      font.nGlyphs =
        charStringsIndex.offsets.length - (header.formatMajor > 1 ? 1 : 0);
    } else {
      charStringsIndex = parseCFFIndex(
        data,
        start + topDict.charStrings,
        null,
        header.formatMajor
    ...
    if (
      header.formatMajor > 1 &&
      font.tables.maxp &&
      font.nGlyphs !== font.tables.maxp.numGlyphs
    ) {
      console.error(
        `Glyph count in the CFF2 table (${font.nGlyphs}) must correspond to the glyph count in the maxp table (${font.tables.maxp.numGlyphs})`
      );
    }
    if (header.formatMajor < 2) {
      let charset = [];
    ...
    if (opt.lowMemory) {
      font._push = function (i) {
        ...
        font.glyphs.push(
          i,
          glyphset_default.cffGlyphLoader(
            font,
            i,
            parseCFFCharstring,
            charString,
  ...
  subtableMakers[2] = function makeLookup2(subtable) {
    ...
    return new table_default.Table(
      ...
      [
        ...
        table_default.tableList(
          "seqSet",
          subtable.sequences,
          function (sequenceSet) {
            return new table_default.Table(
              "sequenceSetTable",
              table_default.ushortList("sequence", sequenceSet)
            );
          }
        )
      )
      ...
      [
        ...
        table_default.tableList(
          ...
          function (alternateSet) {
            return new table_default.Table(
              "alternateSetTable",
              table_default.ushortList("alternate", alternateSet)
            );
          }
        )
      )
  ...
  subtableMakers[5] = function makeLookup5(subtable) {
    if (subtable.substFormat === 1) {
              ...
              return new table_default.Table(
                ...
                table_default.tableList(
                  ...
                  function (sequenceRule) {
                    ...
                    for (
                      let i = 0;
                      i < sequenceRule.lookupRecords.length;
                      i++
                    ) {
              ...
              return new table_default.Table(
                ...
                table_default.tableList(
                  ...
                  function (classSeqRule) {
                    ...
                    for (
                      let i = 0;
                      i < classSeqRule.lookupRecords.length;
                      i++
                    ) {
      ...
      let returnTable = new table_default.Table(
        "contextualSubstitutionTable",
        tableData
      );
      return returnTable;
  ...
  subtableMakers[6] = function makeLookup6(subtable) {
    if (subtable.substFormat === 1) {
      ...
      check_default.assert(
        false,
        "lookup type 6 format 2 is not yet supported."
      );
    } else if (subtable.substFormat === 3) {
      let tableData = [
        { name: "substFormat", type: "USHORT", value: subtable.substFormat },
  ...
  function parseColrTable(data, start) {
    ...
    if (version !== 0) {
      console.warn(
        "Only COLRv0 is currently fully supported. A subset of color glyphs might be available in this font if provided in the v0 format."
      );
    }
  ...
  function parseFvarInstance(data, start, axes, names, instanceSize) {
    ...
    inst.postScriptNameID =
      postScriptNameID == 65535 ? void 0 : postScriptNameID;
    inst.postScriptName =
      inst.postScriptNameID !== void 0
        ? getNameByID(names, postScriptNameID, [6])
        : "";
    return inst;
  }
  function makeFvarTable(fvar, names) {
    ...
    for (let j = 0; j < fvar.instances.length; j++) {
      result.fields = result.fields.concat(
        makeFvarInstance(j, fvar.instances[j], fvar.axes, optionalFields)
      );
    }
    return result;
  }
  ...
  function parseSTATTable(data, start, fvar) {
    ...
    const elidedFallbackNameID =
      tableVersionMajor > 1 || tableVersionMinor > 0 ? p.parseUShort() : void 0;
    if (fvar !== void 0) {
      check_default.argument(
  ...
  function makeAvarTable(avar, fvar) {
    check_default.argument(
      avar.axisSegmentMaps.length === fvar.axes.length,
      "avar axis count must correspond to fvar axis count"
    );
  ...
  function parseAvarTable(data, start, fvar) {
    ...
    if (tableVersionMajor !== 1) {
      console.warn(
        `Unsupported avar table version ${tableVersionMajor}.${tableVersionMinor}`
      );
    }
    p.skip("uShort", 1);
    const axisCount = p.parseUShort();
    check_default.argument(
      axisCount === fvar.axes.length,
      "avar axis count must correspond to fvar axis count"
    );
  ...
  function parseCvarTable(data, start, fvar, cvt) {
    ...
    if (tableVersionMajor !== 1) {
      console.warn(
        `Unsupported cvar table version ${tableVersionMajor}.${tableVersionMinor}`
      );
    }
    return {
      version: [tableVersionMajor, tableVersionMinor],
  ...
  function parseGvarTable(data, start, fvar, glyphs) {
    ...
    if (tableVersionMajor !== 1) {
      console.warn(
        `Unsupported gvar table version ${tableVersionMajor}.${tableVersionMinor}`
      );
    }
    const axisCount = p.parseUShort();
    if (axisCount !== fvar.axes.length) {
      console.warn(
        `axisCount ${axisCount} in gvar table does not match the number of axes ${fvar.axes.length} in the fvar table!`
      );
    }
    ...
    const glyphVariations = p.parseTupleVariationStoreList(
      axisCount,
      "gvar",
      glyphs
    );
  ...
  function parseGaspTable(data, start) {
    ...
    check_default.argument(
      gasp.version <= 1,
      "Unsupported gasp table version."
    );
  ...
  function makeGaspTable(gasp) {
    ...
    for (let i in gasp.numRanges) {
      result.fields.push({
        name: "rangeMaxPPEM",
        type: "USHORT",
        value: gasp.numRanges[i].rangeMaxPPEM,
      });
      result.fields.push({
        name: "rangeGaspBehavior",
        type: "USHORT",
        value: gasp.numRanges[i].rangeGaspBehavior,
      });
    }
    return result;
  }
  ...
  function makeSvgTable(svgTable) {
    ...
    for (let i = 0, l = glyphIds.length; i < l; i++) {
      ...
      if (
        glyphId - 1 === record.endGlyphID &&
        svgDocOffset === record.svgDocOffset
      ) {
    ...
    for (let i = 0; i < numEntries; i++) {
      ...
      const { startGlyphID, endGlyphID, svgDocOffset, svgDocLength } =
        documentRecords[i];
      fields[fieldIndex++] = {
  ...
  function fontToSfntTable(font) {
    ...
    if (!names.unicode.preferredFamily) {
      names.unicode.preferredFamily =
        fontNamesUnicode.fontFamily ||
        fontNamesMacintosh.fontFamily ||
        fontNamesWindows.fontFamily;
    }
    ...
    if (!names.unicode.preferredSubfamily) {
      names.unicode.preferredSubfamily =
        fontNamesUnicode.fontSubfamily ||
        fontNamesMacintosh.fontSubfamily ||
        fontNamesWindows.fontSubfamily;
    }
    ...
    if (!names.windows.preferredSubfamily) {
      names.windows.preferredSubfamily =
        fontNamesWindows.fontSubfamily ||
        fontNamesUnicode.fontSubfamily ||
        fontNamesMacintosh.fontSubfamily;
    }
    ...
    const ltagTable =
      languageTags.length > 0 ? ltag_default.make(languageTags) : void 0;
    ...
    for (let tableName in optionalTables) {
      ...
      if (table) {
        const tableData = optionalTables[tableName].make.call(
          font,
          table,
          ...(optionalTableArgs[tableName] || [])
        );
        if (tableData) {
  ...
  var PaletteManager = class {
    ...
    getAll(colorFormat) {
      ...
      for (let i = 0; i < cpal.colorRecordIndices.length; i++) {
        ...
        for (let j = startIndex; j < startIndex + cpal.numPaletteEntries; j++) {
          paletteColors.push(
            formatColor(cpal.colorRecords[j], colorFormat || "hexa")
          );
        }
    ...
    extend(num) {
      ...
      const palettes = this.getAll().map((palette) =>
        this.fillPalette(palette, [], newCount)
      );
    ...
    deleteColor(colorIndex, replacementIndex) {
      ...
      if (replacementIndex > cpal.numPaletteEntries - 1) {
        throw Error(
          `Replacement index out of range: numPaletteEntries after deletion: ${
      ...
      for (let i = 0; i < palettes.length; i++) {
        ...
        const updatedPalette = palette.filter(
        ...
        for (let i = 0; i < layerRecords.length; i++) {
          ...
          if (currentIndex > colorIndex) {
            ...
            for (let j = 0; j < palettes.length; j++) {
              if (
                replacementIndex > colorIndex &&
                replacementIndex <= colorIndex + palettes[j].length
              ) {
                replacementShift++;
                break;
              }
    ...
    updateIndices() {
      const cpal = this.cpal();
      const paletteCount = Math.ceil(
        cpal.colorRecords.length / cpal.numPaletteEntries
      );
  ...
  var LayerManager = class {
    ...
    add(glyphIndex, layers, position) {
      ...
      if (
        ...
        position =
          currentLayers.length + 1 + (position % (currentLayers.length + 1));
      ...
      for (let i = 0; i < position; i++) {
        const glyphID = Number.isInteger(currentLayers[i].glyph)
          ? currentLayers[i].glyph
          : currentLayers[i].glyph.index;
        newLayers.push({
          glyphID,
      ...
      for (const layer of layers) {
        const glyphID = Number.isInteger(layer.glyph)
          ? layer.glyph
          : layer.glyph.index;
        newLayers.push({
          glyphID,
    ...
    setPaletteIndex(glyphIndex, layerIndex, paletteIndex) {
      ...
      if (layers[layerIndex]) {
        layers = layers.map((layer, index) => ({
          ...
          paletteIndex:
            index === layerIndex ? paletteIndex : layer.paletteIndex,
        }));
    ...
    updateColrTable(glyphIndex, layers) {
      ...
      if (layerDiff > 0) {
        ...
        colr.layerRecords.splice(
          baseGlyphRecord.firstLayerIndex + originalNumLayers,
          0,
          ...newLayers
        );
      } else if (layerDiff < 0) {
        colr.layerRecords.splice(
          baseGlyphRecord.firstLayerIndex + newNumLayers,
          -layerDiff
        );
      }
      ...
      if (layerDiff !== 0) {
        for (let i = 0; i < colr.baseGlyphRecords.length; i++) {
          ...
          if (
            i === index ||
            sibling.firstLayerIndex < baseGlyphRecord.firstLayerIndex
          )
            continue;
  ...
  var SVGImageManager = class {
    ...
    getOrCreateSvgImageCacheEntry(glyphIndex) {
      ...
      if (svgImageCacheEntry === void 0) {
        svgImageCacheEntry = createSvgImageCacheEntry(
          this.font,
          svgDocCacheEntry.template,
          glyphIndex
        );
  ...
  function decodeSvgDocumentWithTinyInflate(buf) {
    try {
      return Promise.resolve(
        new TextDecoder().decode(isGzip(buf) ? unGzip(buf) : buf)
      );
    } catch (error) {
  ...
  function makeSvgTemplate(text) {
    ...
    if (
      / id=['"]glyph\d+['"]/.test(text.substring(documentStart, contentStart))
    ) {
      return text;
    }
  ...
  function makeSvgImage(text, unitsPerEm) {
    ...
    if (viewBoxVal.width > 0 && viewBoxVal.height > 0) {
      if (widthVal.unitType === 1) {
        ...
        yScale =
          heightVal.unitType === 1
            ? heightVal.valueInSpecifiedUnits / viewBoxVal.height
            : xScale;
  ...
  function parseGlyfTableAll(data, start, loca, font) {
    ...
    for (let i = 0; i < loca.length - 1; i += 1) {
      ...
      if (offset !== nextOffset) {
        glyphs.push(
          i,
          glyphset_default.ttfGlyphLoader(
            font,
            i,
            parseGlyph,
            data,
  ...
  var VariationProcessor = class {
    ...
    getNormalizedCoords(coords) {
      ...
      for (let i = 0; i < this.fvar().axes.length; i++) {
        ...
        if (tagValue < axis.defaultValue) {
          normalized.push(
            (tagValue - axis.defaultValue + Number.EPSILON) /
        ...
        for (let i = 0; i < this.avar().axisSegmentMaps.length; i++) {
          ...
          for (let j = 0; j < segment.axisValueMaps.length; j++) {
            ...
            if (j >= 1 && normalized[i] < pair.fromCoordinate) {
              ...
              normalized[i] =
                ((normalized[i] - prev.fromCoordinate) *
                  (pair.toCoordinate - prev.toCoordinate) +
                  Number.EPSILON) /
    ...
    transformComponents(
      ...
      for (let c = 0; c < glyph.components.length; c++) {
        ...
        if (deltaIndex > -1) {
          componentTransform.dx += Math.round(
            header.deltas[deltaIndex] * factor
          );
          componentTransform.dy += Math.round(
            header.deltasY[deltaIndex] * factor
          );
        }
        const transformedComponentPoints = transformPoints(
          this.getTransform(componentGlyph, coords).points,
          componentTransform
        );
        transformedPoints.splice(
          pointsIndex,
          transformedComponentPoints.length,
          ...transformedComponentPoints
        );
      ...
      for (let h = 0; h < headers.length; h++) {
        ...
        for (let a = 0; a < axisCount; a++) {
          ...
          switch (flavor) {
            case "gvar":
              tupleCoords = header.peakTuple
                ? header.peakTuple
                : this.gvar().sharedTuples[header.sharedTupleRecordsIndex];
              break;
            case "cvar":
              tupleCoords = header.peakTuple;
              break;
          ...
          if (!header.intermediateStartTuple) {
            if (
              normalizedCoords[a] < Math.min(0, tupleCoords[a]) ||
            ...
            factor =
              (factor * normalizedCoords[a] + Number.EPSILON) /
              (tupleCoords[a] + Number.EPSILON);
          } else {
            if (
              normalizedCoords[a] < header.intermediateStartTuple[a] ||
              normalizedCoords[a] > header.intermediateEndTuple[a]
            ) {
              factor = 0;
              break;
              ...
              factor =
                (factor *
                  (normalizedCoords[a] -
                    header.intermediateStartTuple[a] +
                    Number.EPSILON)) /
                (tupleCoords[a] -
                  header.intermediateStartTuple[a] +
                  Number.EPSILON);
        ...
        const tuplePoints = header.privatePoints.length
          ? header.privatePoints
          : sharedPoints;
        if (flavor === "gvar" && args.glyph && args.glyph.isComposite) {
          ...
          for (let i = 0; i < transformedPoints.length; i++) {
            ...
            if (flavor === "gvar") {
              ...
              transformedPoints[i] = Math.round(
                point + header.deltas[i] * factor
              );
            }
          }
        } else {
          ...
          for (let i = 0; i < tuplePoints.length; i++) {
            ...
            if (pointIndex < points.length) {
              ...
              if (flavor === "gvar") {
                ...
                transformedPoints[pointIndex] = Math.round(
                  point + header.deltas[i] * factor
                );
              }
            }
          ...
          if (flavor === "gvar") {
            ...
            for (let i = 0; i < points.length; i++) {
              ...
              transformedPoints[i].x = Math.round(
                transformedPoints[i].x + deltaX
              );
              transformedPoints[i].y = Math.round(
    ...
    getTransform(glyph, coords) {
      ...
      if (hasBlend || hasPoints) {
        ...
        if (hasPoints) {
          const variationData =
            this.gvar() && this.gvar().glyphVariations[glyph.index];
          if (variationData) {
            const glyphPoints = glyph.points;
            let transformedPoints = this.applyTupleVariationStore(
              variationData,
              glyphPoints,
              coords,
              "gvar",
            ...
            transformedGlyph = new glyph_default(
              Object.assign({}, glyph, {
          ...
          transformedGlyph = new glyph_default(
            Object.assign({}, glyph, { path: blendPath })
          );
        }
      }
      if (this.font.tables.hvar) {
        glyph._advanceWidth =
          typeof glyph._advanceWidth !== "undefined"
            ? glyph._advanceWidth
            : glyph.advanceWidth;
        glyph.advanceWidth = transformedGlyph.advanceWidth = Math.round(
          glyph._advanceWidth +
            this.getVariableAdjustment(
              transformedGlyph.index,
              "hvar",
              "advanceWidth",
              coords
            )
        );
        glyph._leftSideBearing =
          typeof glyph._leftSideBearing !== "undefined"
            ? glyph._leftSideBearing
            : glyph.leftSideBearing;
        glyph.leftSideBearing = transformedGlyph.leftSideBearing = Math.round(
          glyph._leftSideBearing +
            this.getVariableAdjustment(
              transformedGlyph.index,
              "hvar",
    ...
    getVariableAdjustment(gid, tableName, parameter, coords) {
      ...
      if (!table) {
        throw Error(
          `trying to get variation adjustment from non-existent table "${table}"`
        );
      }
      if (!table.itemVariationStore) {
        throw Error(
          `trying to get variation adjustment from table "${table}" which does not have an itemVariationStore`
        );
      }
  ...
  var VariationManager = class {
    ...
    getDefaultInstanceIndex() {
      ...
      if (defaultInstanceIndex < 0) {
        defaultInstanceIndex = this.fvar().instances.findIndex(
          (instance) => instance.name && instance.name.en === "Regular"
        );
      }
  ...
  Hinting.prototype.exec = function (glyph, ppem) {
    ...
    if (!prepState || prepState.ppem !== ppem) {
      ...
      prepState.ppem = ppem;
      const oCvt =
        (font.variation && font.variation.process.getCvarTransform()) ||
        font.tables.cvt;
      if (oCvt) {
        ...
        if (matched) {
          ...
          const parser = (substitutions2, lookupRecord) => {
            ...
            for (const subtable2 of subtables) {
              let ligSetIndex2 = lookupCoverage(
                substitutions2[sequenceIndex],
                subtable2.coverage
              );
              if (ligSetIndex2 !== -1) {
  ...
  function contextSubstitutionFormat3(contextParams, subtable) {
    ...
    for (let i = 0; i < subtable.coverages.length; i++) {
      ...
      const coverage = subtable.coverages[i];
      let glyphIndex =
        contextParams.context[contextParams.index + lookupRecord.sequenceIndex];
  ...
  FeatureQuery.prototype.getLookupMethod = function (lookupTable, subtable) {
    ...
    switch (substitutionType) {
      case "11":
        return (glyphIndex) =>
          singleSubstitutionFormat1.apply(this, [glyphIndex, subtable]);
      case "12":
        return (glyphIndex) =>
          singleSubstitutionFormat2.apply(this, [glyphIndex, subtable]);
      case "63":
        return (contextParams) =>
          chainingSubstitutionFormat3.apply(this, [contextParams, subtable]);
      case "41":
        return (contextParams) =>
          ligatureSubstitutionFormat1.apply(this, [contextParams, subtable]);
      case "21":
        return (glyphIndex) =>
          decompositionSubstitutionFormat1.apply(this, [glyphIndex, subtable]);
      case "51":
        return (contextParams) =>
          contextSubstitutionFormat1.apply(this, [contextParams, subtable]);
      case "53":
        return (contextParams) =>
          contextSubstitutionFormat3.apply(this, [contextParams, subtable]);
      default:
        throw new Error(
  ...
  FeatureQuery.prototype.lookupFeature = function (query) {
    ...
    if (!feature)
      return new Error(
        `font '${
          (
            this.font.names.unicode ||
            this.font.names.windows ||
            this.font.names.macintosh
          ).fullName.en
        }' doesn't support feature '${query.tag}' for script '${query.script}'.`
      );
  ...
  FeatureQuery.prototype.supports = function (query) {
    ...
    const supportedScript = Object.prototype.hasOwnProperty.call(
      this.features,
      query.script
    );
    if (!query.tag) return supportedScript;
    const supportedFeature = this.features[query.script].some(
      (feature) => feature.tag === query.tag
    );
  ...
  Bidi.prototype.registerFeatures = function (script, tags) {
    const supportedTags = tags.filter((tag) =>
      this.query.supports({ script, tag })
    );
  ...
  function Font(options) {
    ...
    if (!options.empty) {
      if (!options.familyName)
        throw "When creating a new Font object, familyName is required.";
      if (!options.styleName)
        throw "When creating a new Font object, styleName is required.";
      if (!options.unitsPerEm)
        throw "When creating a new Font object, unitsPerEm is required.";
      if (!options.ascender)
        throw "When creating a new Font object, ascender is required.";
      if (options.descender > 0)
        throw "When creating a new Font object, negative descender value is required.";
      this.names = {};
  ...
  Font.prototype.getEnglishName = function (name) {
    const translations = (this.names.unicode ||
      this.names.macintosh ||
      this.names.windows)[name];
    if (translations) {
  ...
  Font.prototype.validate = function () {
    ...
    if (this.tables.colr) {
      ...
      for (let b = 0; b < baseGlyphs.length; b++) {
        ...
        assert(
          previousID < baseGlyphs[b].glyphID,
          `baseGlyphs must be sorted by GlyphID in ascending order, but glyphID ${currentGlyphID} comes after ${previousID}`
        );
  ...
  Font.prototype.download = function () {
    console.error(
      "DEPRECATED: platform-specific actions are to be implemented on user-side"
    );
  };
  ...
  function parseHvarTable(data, start) {
    ...
    if (tableVersionMajor !== 1) {
      console.warn(
        `Unsupported hvar table version ${tableVersionMajor}.${tableVersionMinor}`
      );
    }
    const version = [tableVersionMajor, tableVersionMinor];
  ...
  function parseBuffer(buffer, opt = {}) {
    ...
    font.tables.name = name_default.parse(
      nameTable.data,
      nameTable.offset,
      ltagTable
    );
    ...
    if (glyfTableEntry && locaTableEntry) {
      ...
      const locaOffsets = loca_default.parse(
        locaTable.data,
        locaTable.offset,
        font.numGlyphs,
        shortVersion
      ...
      font.glyphs = glyf_default.parse(
        glyfTable.data,
        glyfTable.offset,
        locaOffsets,
        font,
    ...
    if (fvarTableEntry) {
      ...
      font.tables.fvar = fvar_default.parse(
        fvarTable.data,
        fvarTable.offset,
        font.names
      );
    ...
    if (statTableEntry) {
      ...
      font.tables.stat = stat_default.parse(
        statTable.data,
        statTable.offset,
        font.tables.fvar
      );
    ...
    if (gvarTableEntry) {
      if (!fvarTableEntry) {
        console.warn(
          "This font provides a gvar table, but no fvar table, which is required for variable fonts."
        );
      }
      if (!glyfTableEntry) {
        console.warn(
          "This font provides a gvar table, but no glyf table. Glyph variation only works with TrueType outlines."
        );
      }
      const gvarTable = uncompressTable(data, gvarTableEntry);
      font.tables.gvar = gvar_default.parse(
        gvarTable.data,
        gvarTable.offset,
        font.tables.fvar,
        font.glyphs
    ...
    if (cvarTableEntry) {
      if (!fvarTableEntry) {
        console.warn(
          "This font provides a cvar table, but no fvar table, which is required for variable fonts."
        );
      }
      if (!font.tables.cvt) {
        console.warn(
          "This font provides a cvar table, but no cvt table which could be made variable."
        );
      }
      if (!glyfTableEntry) {
        console.warn(
      ...
      font.tables.cvar = cvar_default.parse(
        cvarTable.data,
        cvarTable.offset,
        font.tables.fvar,
        font.tables.cvt || []
    ...
    if (avarTableEntry) {
      if (!fvarTableEntry) {
        console.warn(
          "This font provides an avar table, but no fvar table, which is required for variable fonts."
        );
      }
      const avarTable = uncompressTable(data, avarTableEntry);
      font.tables.avar = avar_default.parse(
        avarTable.data,
        avarTable.offset,
        font.tables.fvar
      );
    ...
    if (hvarTableEntry) {
      if (!fvarTableEntry) {
        console.warn(
          "This font provides an HVAR table, but no fvar table, which is required for variable fonts."
        );
      }
      if (!hmtxTableEntry) {
        console.warn(
          "This font provides an HVAR table, but no hmtx table to vary."
        );
      }
      const hvarTable = uncompressTable(data, hvarTableEntry);
      font.tables.hvar = hvar_default.parse(
        hvarTable.data,
        hvarTable.offset,
        font.tables.fvar
      );
  ...
  function load() {
    console.error(
      "DEPRECATED! migrate to: opentype.parse(buffer, opt) See: https://github.com/opentypejs/opentype.js/issues/675"
    );
  }
  function loadSync() {
    console.error(
